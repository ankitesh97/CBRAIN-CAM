...
def return_from_dataset(ds,var):
    return ds[var][1:]

def compute_TfromNS(ds):
    return compute_bp(ds,'TBP')-compute_bp(ds,'TBP')[:,-1,:,:]

def compute_TBCTEND(ds):
    TBCTEND = (ds.TBC-ds.TBP)/DT
    return TBCTEND[1:]
    
def compute_QBCTEND(ds):
    QBCTEND = (data.QBC-data.QBP)/DT
    return QBCTEND[1:]

def compute_CLDLIQBCTEND(ds):
    CLDLIQBCTEND = (data.CLDLIQBC-data.CLDLIQBP)/DT
    return CLDLIQBCTEND[1:]
    
def compute_CLDICETEND(ds):
    CLDICEBCTEND = (data.CLDICEBC-data.CLDICEBP)/DT
    return CLDICEBCTEND[1:]
...
def create_stacked_da(ds, vars):
    ...
    for var in vars:
        ...
        if 'EPS' in var:
            ...
            da = compute_TBCTEND(ds)
        elif var == 'QBCTEND':
            da = compute_QBCTEND(ds)
        elif var == 'CLDLIQBCTEND':
            da = compute_CLDLIQBCTEND(ds)
        elif var == 'CLDICETEND':
            da = compute_CLDICETEND(ds)
