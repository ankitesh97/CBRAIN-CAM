...
class DataGeneratorClassification(tf.keras.utils.Sequence):
    ...
    def _transform_to_all_flattend(self,Y):
        Y_trans = []
        ...
        return Y_concatenated.reshape(-1,64*(self.bin_size+2))
...
densout = LeakyReLU(alpha=0.3)(densout)
densout = Dense(2048, activation='linear')(densout)
densout = LeakyReLU(alpha=0.3)(densout)
densout = Dense(4096, activation='linear')(densout)
densout = LeakyReLU(alpha=0.3)(densout)
...
mcp_save = ModelCheckpoint(path_HDF5+'BF_Classification_bin_size_100_model_v2_03.hdf5',save_best_only=True, monitor='val_loss', mode='min')
